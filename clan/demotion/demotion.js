...
module.exports = (message,client,utente,leader) => {
    ...
    if(utente == undefined){
        ...
        if(utente.user.bot != true){ 
            if(utente.roles.cache.has(config.grado2)){
                if(message.member.roles.cache.has(leader) || message.member.roles.cache.has(config.grado3)){
                    const embed = new MessageEmbed()
                        ...
                        .setDescription("🔰 <@!" + utente + "> è stato retrocesso a <@&" + message.guild.roles.cache.get(config.grado1) + "> 🔰")
                        .setColor(0x00d000)                                  
                    message.guild.members.cache.get(utente.id).roles.remove(config.grado2).catch(console.error); 
                    message.guild.members.cache.get(utente.id).roles.add(config.grado1).catch(console.error);                      
                ...
                if(message.member.roles.cache.has(leader)){
                    const embed = new MessageEmbed()
                        .setTitle("🛡️ Statua Corrotta al suo servizio! 🛡️")
                        .setDescription("🔰 <@!"+ utente +"> è stato retrocesso a <@&" + message.guild.roles.cache.get(config.grado2) + "> 🔰")
                        .setColor(0x00d000)                                  
